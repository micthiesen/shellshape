...
func TestStrace(t *testing.T) {
	...
	t.Run("different commands collide", func(t *testing.T) {
		a := Normalize("strace -f ./myapp --foo")
		b := Normalize("strace -f /usr/bin/other --bar baz")
		if a != b {
	...
	t.Run("different pids collide", func(t *testing.T) {
		a := Normalize("strace -p 1234")
		b := Normalize("strace -p 5678")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("strace ls")
		subshell := Normalize("strace $(dangerous-command)")
		if benign == subshell {
...
func TestDtrace(t *testing.T) {
	...
	t.Run("different probes collide", func(t *testing.T) {
		a := Normalize("dtrace -n 'syscall::read:entry'")
		b := Normalize("dtrace -n 'io:::start'")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("dtrace -c ./myapp")
		subshell := Normalize("dtrace -c $(dangerous-command)")
		if benign == subshell {
			t.Error("subshell must produce different shape than literal")
		}
	})
