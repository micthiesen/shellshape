...
func TestPerl(t *testing.T) {
	...
	t.Run("different scripts collide", func(t *testing.T) {
		a := Normalize("perl deploy.pl")
		b := Normalize("perl backup.pl")
		if a != b {
	...
	t.Run("different expressions collide", func(t *testing.T) {
		a := Normalize("perl -pe 's/foo/bar/g' data.txt")
		b := Normalize("perl -pe 's/hello/world/g' notes.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("perl script.pl")
		subshell := Normalize("perl $(dangerous-command)")
		if benign == subshell {
