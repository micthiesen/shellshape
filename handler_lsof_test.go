...
func TestLsof(t *testing.T) {
	...
	t.Run("different PIDs collide", func(t *testing.T) {
		a := Normalize("lsof -p 1234")
		b := Normalize("lsof -p 9999")
		if a != b {
	...
	t.Run("different users collide", func(t *testing.T) {
		a := Normalize("lsof -u root")
		b := Normalize("lsof -u www-data")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different network specs collide", func(t *testing.T) {
		a := Normalize("lsof -i :8080")
		b := Normalize("lsof -i :3000")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("lsof /var/log/syslog")
		subshell := Normalize("lsof $(dangerous-command)")
		if benign == subshell {
