...
func TestUseradd(t *testing.T) {
	...
	t.Run("different usernames distinct", func(t *testing.T) {
		a := Normalize("useradd alice")
		b := Normalize("useradd bob")
		if a == b {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := Normalize("useradd -d /home/alice alice")
		b := Normalize("useradd -d /opt/deploy deploy")
		if a == b {
			t.Errorf("expected different shapes for different usernames")
		}
		// But same username with different paths should collide
		c := Normalize("useradd -d /home/custom john")
		d := Normalize("useradd -d /opt/custom john")
		if c != d {
	...
	t.Run("different uids collide", func(t *testing.T) {
		a := Normalize("useradd -u 1001 john")
		b := Normalize("useradd -u 5000 john")
		if a != b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("useradd literal-user")
		subshell := Normalize("useradd $(dangerous-command)")
		if benign == subshell {
