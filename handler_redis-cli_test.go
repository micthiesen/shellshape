...
func TestRedisCli(t *testing.T) {
	...
	t.Run("different keys collide", func(t *testing.T) {
		a := Normalize("redis-cli GET user:123")
		b := Normalize("redis-cli GET session:abc")
		if a != b {
	...
	t.Run("different hosts collide", func(t *testing.T) {
		a := Normalize("redis-cli -h 10.0.0.1 -p 6379 PING")
		b := Normalize("redis-cli -h 192.168.1.100 -p 6380 PING")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different passwords collide", func(t *testing.T) {
		a := Normalize("redis-cli -a secret1 GET key1")
		b := Normalize("redis-cli -a secret2 GET key2")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("redis-cli GET mykey")
		subshell := Normalize("redis-cli $(dangerous-command)")
		if benign == subshell {
