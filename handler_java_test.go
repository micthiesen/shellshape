...
func TestJava(t *testing.T) {
	...
	t.Run("different jars collide", func(t *testing.T) {
		a := Normalize("java -jar server.jar")
		b := Normalize("java -jar worker.jar")
		if a != b {
	...
	t.Run("different args collide", func(t *testing.T) {
		a := Normalize("java -jar app.jar --port 3000")
		b := Normalize("java -jar app.jar --host localhost")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("java -jar app.jar")
		subshell := Normalize("java -jar $(dangerous-command)")
		if benign == subshell {
...
func TestJavac(t *testing.T) {
	...
	t.Run("different source files collide", func(t *testing.T) {
		a := Normalize("javac Main.java")
		b := Normalize("javac App.java")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different classpaths collide", func(t *testing.T) {
		a := Normalize("javac -cp lib/a.jar Main.java")
		b := Normalize("javac -cp lib/b.jar App.java")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	// SAFETY TEST
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("javac Main.java")
		subshell := Normalize("javac $(dangerous-command)")
		if benign == subshell {
			t.Error("subshell must produce different shape than literal")
		}
	})
