...
func TestFmt(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("fmt readme.txt")
		b := Normalize("fmt letter.md")
		if a != b {
	...
	t.Run("different widths collide", func(t *testing.T) {
		a := Normalize("fmt -w 72 file.txt")
		b := Normalize("fmt -w 80 file.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("fmt literal-arg")
		subshell := Normalize("fmt $(dangerous-command)")
		if benign == subshell {
