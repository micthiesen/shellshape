...
func TestGzip(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("gzip -9 /var/log/syslog")
		b := Normalize("gzip -9 /tmp/data.csv")
		if a != b {
	...
	t.Run("different suffixes collide", func(t *testing.T) {
		a := Normalize("gzip -S .bak file.txt")
		b := Normalize("gzip -S .old file.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("gzip literal-arg")
		subshell := Normalize("gzip $(dangerous-command)")
		if benign == subshell {
