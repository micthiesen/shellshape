...
func TestShasum(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("shasum -a 256 /etc/passwd")
		b := Normalize("shasum -a 256 /var/log/syslog")
		if a != b {
	...
	t.Run("different file counts collide", func(t *testing.T) {
		a := Normalize("shasum file1.txt file2.txt")
		b := Normalize("shasum a.txt b.txt c.txt d.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("shasum literal-file")
		subshell := Normalize("shasum $(dangerous-command)")
		if benign == subshell {
