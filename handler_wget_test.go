...
func TestWget(t *testing.T) {
	...
	t.Run("different URLs collide", func(t *testing.T) {
		a := Normalize("wget https://example.com/file1.tar.gz")
		b := Normalize("wget https://other-site.org/different-file.zip")
		if a != b {
	...
	t.Run("different output files collide", func(t *testing.T) {
		a := Normalize("wget -O /tmp/output1.txt https://example.com/a")
		b := Normalize("wget -O /home/user/result.json https://example.com/b")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("wget https://example.com/file")
		subshell := Normalize("wget $(dangerous-command)")
		if benign == subshell {
