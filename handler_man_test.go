...
func TestMan(t *testing.T) {
	...
	t.Run("different manpaths collide", func(t *testing.T) {
		a := Normalize("man -M /usr/local/man ls")
		b := Normalize("man -M /opt/man ls")
		if a != b {
	...
	t.Run("different pagers collide", func(t *testing.T) {
		a := Normalize("man -P less ls")
		b := Normalize("man -P more ls")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("man ls")
		subshell := Normalize("man $(dangerous-command)")
		if benign == subshell {
