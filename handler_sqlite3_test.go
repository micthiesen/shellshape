...
func TestSqlite3(t *testing.T) {
	...
	t.Run("different queries collide", func(t *testing.T) {
		a := Normalize(`sqlite3 data.db "SELECT * FROM users"`)
		b := Normalize(`sqlite3 data.db "DELETE FROM sessions WHERE expired = 1"`)
		if a != b {
	...
	t.Run("different databases collide", func(t *testing.T) {
		a := Normalize(`sqlite3 /tmp/a.db "SELECT 1"`)
		b := Normalize(`sqlite3 /tmp/b.db "SELECT 1"`)
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize(`sqlite3 data.db "SELECT 1"`)
		subshell := Normalize(`sqlite3 data.db $(cat query.sql)`)
		if benign == subshell {
