...
func TestApt(t *testing.T) {
	...
	t.Run("different search terms collide", func(t *testing.T) {
		a := Normalize("apt search postgres")
		b := Normalize("apt search redis")
		if a != b {
	...
	t.Run("different search terms collide apt-cache", func(t *testing.T) {
		a := Normalize("apt-cache search nginx")
		b := Normalize("apt-cache search memcached")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("apt install literal-arg")
		subshell := Normalize("apt install $(dangerous-command)")
		if benign == subshell {
