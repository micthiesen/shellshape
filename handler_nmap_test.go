...
func TestNmap(t *testing.T) {
	...
	t.Run("different targets collide", func(t *testing.T) {
		a := Normalize("nmap -sS -p 80 192.168.1.1")
		b := Normalize("nmap -sS -p 80 10.0.0.5")
		if a != b {
	...
	t.Run("different ports collide", func(t *testing.T) {
		a := Normalize("nmap -p 22,80 192.168.1.1")
		b := Normalize("nmap -p 443,8080 10.0.0.1")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("nmap 192.168.1.1")
		subshell := Normalize("nmap $(cat targets.txt)")
		if benign == subshell {
