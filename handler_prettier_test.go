...
func TestPrettier(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("prettier --write src/app.tsx")
		b := Normalize("prettier --write lib/utils.js")
		if a != b {
	...
	t.Run("different parser values collide", func(t *testing.T) {
		a := Normalize("prettier --parser typescript --write src/")
		b := Normalize("prettier --parser babel --write lib/")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("prettier --write file.js")
		subshell := Normalize("prettier --write $(find . -name '*.js')")
		if benign == subshell {
