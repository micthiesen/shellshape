...
func TestVagrant(t *testing.T) {
	...
	t.Run("different box names collide", func(t *testing.T) {
		a := Normalize("vagrant box add hashicorp/precise32")
		b := Normalize("vagrant box add ubuntu/focal64")
		if a != b {
	...
	t.Run("different provider values collide", func(t *testing.T) {
		a := Normalize("vagrant up --provider virtualbox")
		b := Normalize("vagrant up --provider vmware_fusion")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different snapshot names collide", func(t *testing.T) {
		a := Normalize("vagrant snapshot save before-update")
		b := Normalize("vagrant snapshot save clean-state")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("vagrant up myvm")
		subshell := Normalize("vagrant up $(dangerous-command)")
		if benign == subshell {
