...
func TestJust(t *testing.T) {
	...
	t.Run("different recipes collide", func(t *testing.T) {
		a := Normalize("just build")
		b := Normalize("just test")
		if a != b {
	...
	t.Run("different recipe args collide", func(t *testing.T) {
		a := Normalize("just deploy staging")
		b := Normalize("just deploy production")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different justfile paths collide", func(t *testing.T) {
		a := Normalize("just -f Justfile.dev build")
		b := Normalize("just -f Justfile.prod build")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("just build")
		subshell := Normalize("just $(dangerous-command)")
		if benign == subshell {
