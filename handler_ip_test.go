...
func TestIP(t *testing.T) {
	...
	t.Run("different IPs collide", func(t *testing.T) {
		a := Normalize("ip addr add 192.168.1.100/24 dev eth0")
		b := Normalize("ip addr add 10.0.0.5/32 dev wlan0")
		if a != b {
	...
	t.Run("different gateways collide", func(t *testing.T) {
		a := Normalize("ip route add default via 192.168.1.1")
		b := Normalize("ip route add default via 10.0.0.1")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different devices collide", func(t *testing.T) {
		a := Normalize("ip link set dev eth0 up")
		b := Normalize("ip link set dev wlan0 up")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("ip addr add 192.168.1.1/24 dev eth0")
		subshell := Normalize("ip addr add $(dangerous-command) dev eth0")
		if benign == subshell {
