...
func TestDotnet(t *testing.T) {
	...
	t.Run("different configurations collide", func(t *testing.T) {
		a := Normalize("dotnet build -c Release")
		b := Normalize("dotnet build -c Debug")
		if a != b {
	...
	t.Run("different output paths collide", func(t *testing.T) {
		a := Normalize("dotnet publish -o ./publish/linux")
		b := Normalize("dotnet publish -o ./publish/windows")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different test filters collide", func(t *testing.T) {
		a := Normalize("dotnet test --filter FullyQualifiedName~TestA")
		b := Normalize("dotnet test --filter FullyQualifiedName~TestB")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("dotnet literal-arg")
		subshell := Normalize("dotnet $(dangerous-command)")
		if benign == subshell {
