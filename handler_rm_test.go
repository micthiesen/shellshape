...
func TestRm(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("rm -rf /tmp/build")
		b := Normalize("rm -rf /var/log/old")
		c := Normalize("rm -rf ~/Downloads/junk")
		if a != b || b != c {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("rm literal-arg")
		subshell := Normalize("rm $(dangerous-command)")
		if benign == subshell {
