...
func TestStow(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := Normalize("stow -d ~/.dotfiles -t /home/user zsh")
		b := Normalize("stow -d /opt/stow -t /usr/local zsh")
		if a != b {
	...
	t.Run("different ignore patterns collide", func(t *testing.T) {
		a := Normalize("stow --ignore='.git' zsh")
		b := Normalize("stow --ignore='.svn' zsh")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("stow zsh")
		subshell := Normalize("stow $(dangerous-command)")
		if benign == subshell {
