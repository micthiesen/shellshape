...
func TestAwk(t *testing.T) {
	...
	t.Run("different programs collide", func(t *testing.T) {
		a := Normalize("awk '{print $1}' data.csv")
		b := Normalize("awk '{print $NF}' data.csv")
		if a != b {
	...
	t.Run("different field separators collide", func(t *testing.T) {
		a := Normalize("awk -F ',' '{print $1}' file.csv")
		b := Normalize("awk -F ':' '{print $1}' file.csv")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("awk literal-arg")
		subshell := Normalize("awk $(dangerous-command)")
		if benign == subshell {
