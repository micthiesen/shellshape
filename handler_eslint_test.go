...
func TestEslint(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := Normalize("eslint --fix src/app.ts")
		b := Normalize("eslint --fix lib/utils.js")
		if a != b {
	...
	t.Run("different rules collide", func(t *testing.T) {
		a := Normalize("eslint --rule 'no-console: error' src/")
		b := Normalize("eslint --rule 'no-unused-vars: warn' src/")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("eslint src/app.ts")
		subshell := Normalize("eslint $(dangerous-command)")
		if benign == subshell {
