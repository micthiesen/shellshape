...
func TestVitest(t *testing.T) {
	...
	t.Run("different test files collide", func(t *testing.T) {
		a := Normalize("vitest run src/foo.test.ts")
		b := Normalize("vitest run src/bar.test.ts")
		if a != b {
	...
	t.Run("different patterns collide", func(t *testing.T) {
		a := Normalize("vitest -t 'should render button'")
		b := Normalize("vitest -t 'should handle click'")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different numeric values collide", func(t *testing.T) {
		a := Normalize("vitest --bail 1 --retry 3")
		b := Normalize("vitest --bail 5 --retry 10")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("vitest run src/utils.test.ts")
		subshell := Normalize("vitest run $(dangerous-command)")
		if benign == subshell {
