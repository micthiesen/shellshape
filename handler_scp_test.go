...
func TestScp(t *testing.T) {
	...
	t.Run("different hosts collide", func(t *testing.T) {
		a := Normalize("scp file.txt alice@prod-server:/opt/app/")
		b := Normalize("scp file.txt bob@staging-host:/var/data/")
		if a != b {
	...
	t.Run("different ports collide", func(t *testing.T) {
		a := Normalize("scp -P 22 file.txt user@host:/tmp/")
		b := Normalize("scp -P 8022 file.txt user@host:/tmp/")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("scp file.txt user@host:/tmp/")
		subshell := Normalize("scp $(dangerous-command) user@host:/tmp/")
		if benign == subshell {
