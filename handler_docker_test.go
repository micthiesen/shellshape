...
func TestDocker(t *testing.T) {
	...
	t.Run("different port mappings collide", func(t *testing.T) {
		a := Normalize("docker run -p 8080:80 nginx")
		b := Normalize("docker run -p 3000:3000 nginx")
		if a != b {
	...
	t.Run("different env values collide", func(t *testing.T) {
		a := Normalize("docker run -e FOO=bar nginx")
		b := Normalize("docker run -e BAZ=qux nginx")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different volume paths collide", func(t *testing.T) {
		a := Normalize("docker run -v /home/user/data:/data nginx")
		b := Normalize("docker run -v /tmp/test:/app nginx")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("docker run literal-arg")
		subshell := Normalize("docker run $(dangerous-command)")
		if benign == subshell {
	...
	t.Run("subshell as standalone flag value preserved", func(t *testing.T) {
		benign := Normalize("docker run --name myapp nginx")
		subshell := Normalize("docker run --name $(generate-name) nginx")
		if benign == subshell {
