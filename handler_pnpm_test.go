...
func TestPnpm(t *testing.T) {
	...
	t.Run("different packages collide", func(t *testing.T) {
		a := Normalize("pnpm add react express lodash")
		b := Normalize("pnpm add axios moment dayjs")
		if a != b {
	...
	t.Run("different package versions collide", func(t *testing.T) {
		a := Normalize("pnpm add react@18.2.0")
		b := Normalize("pnpm add react@17.0.0")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("pnpm add react")
		subshell := Normalize("pnpm add $(dangerous-command)")
		if benign == subshell {
