...
func TestBlkid(t *testing.T) {
	...
	t.Run("different devices collide", func(t *testing.T) {
		a := Normalize("blkid -s UUID /dev/sda1")
		b := Normalize("blkid -s UUID /dev/nvme0n1p2")
		if a != b {
	...
	t.Run("different labels collide", func(t *testing.T) {
		a := Normalize("blkid -L boot")
		b := Normalize("blkid -L rootfs")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("blkid /dev/sda1")
		subshell := Normalize("blkid $(dangerous-command)")
		if benign == subshell {
