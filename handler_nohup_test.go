...
func TestNohup(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := Normalize("nohup /opt/app/server")
		b := Normalize("nohup /usr/local/bin/daemon")
		if a != b {
	...
	t.Run("different script names collide", func(t *testing.T) {
		a := Normalize("nohup ./start.sh")
		b := Normalize("nohup ./deploy.sh")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("nohup literal-arg")
		subshell := Normalize("nohup $(dangerous-command)")
		if benign == subshell {
