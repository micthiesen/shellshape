...
func TestFfmpeg(t *testing.T) {
	...
	t.Run("different inputs collide", func(t *testing.T) {
		a := Normalize("ffmpeg -i /path/to/video.mp4 output.avi")
		b := Normalize("ffmpeg -i /other/file.mkv result.mp4")
		if a != b {
	...
	t.Run("different bitrates collide", func(t *testing.T) {
		a := Normalize("ffmpeg -i in.mp4 -b:v 2M out.mp4")
		b := Normalize("ffmpeg -i in.mp4 -b:v 5M out.mp4")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("ffmpeg -i input.mp4 output.mp4")
		subshell := Normalize("ffmpeg -i $(echo input.mp4) output.mp4")
		if benign == subshell {
