...
func TestChown(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := Normalize("chown root:wheel /home/alice/file.txt")
		b := Normalize("chown root:wheel /opt/deploy/config.yml")
		if a != b {
	...
	t.Run("different owners distinct", func(t *testing.T) {
		a := Normalize("chown alice file.txt")
		b := Normalize("chown bob file.txt")
		if a == b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("chown root file.txt")
		subshell := Normalize("chown $(dangerous-command) file.txt")
		if benign == subshell {
