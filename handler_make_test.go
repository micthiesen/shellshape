...
func TestMake(t *testing.T) {
	...
	t.Run("different variable values collide", func(t *testing.T) {
		a := Normalize("make build CC=gcc")
		b := Normalize("make build CC=clang")
		if a != b {
	...
	t.Run("different job counts collide", func(t *testing.T) {
		a := Normalize("make build -j 4")
		b := Normalize("make build -j 16")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different makefiles collide", func(t *testing.T) {
		a := Normalize("make build -f Makefile.dev")
		b := Normalize("make build -f Makefile.prod")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("make literal-arg")
		subshell := Normalize("make $(dangerous-command)")
		if benign == subshell {
