...
func TestTsgo(t *testing.T) {
	...
	t.Run("different projects collide", func(t *testing.T) {
		a := Normalize("tsgo -p tsconfig.json --noEmit")
		b := Normalize("tsgo -p tsconfig.build.json --noEmit")
		if a != b {
	...
	t.Run("different target values collide", func(t *testing.T) {
		a := Normalize("tsgo --target ES2020 src/index.ts")
		b := Normalize("tsgo --target ES2022 src/main.ts")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("tsgo src/index.ts")
		subshell := Normalize("tsgo $(dangerous-command)")
		if benign == subshell {
