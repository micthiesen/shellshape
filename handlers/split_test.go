...
func TestSplit(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("split -l 1000 server.log")
		b := shellshape.Normalize("split -l 1000 access.log")
		if a != b {
	...
	t.Run("different line counts collide", func(t *testing.T) {
		a := shellshape.Normalize("split -l 100 file.txt")
		b := shellshape.Normalize("split -l 5000 file.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different prefixes collide", func(t *testing.T) {
		a := shellshape.Normalize("split -l 100 file.txt chunk_")
		b := shellshape.Normalize("split -l 100 file.txt part_")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("split literal-arg")
		subshell := shellshape.Normalize("split $(dangerous-command)")
		if benign == subshell {
...
func TestCsplit(t *testing.T) {
	...
	// COLLISION TESTS
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("csplit server.log '/^Error/' '{*}'")
		b := shellshape.Normalize("csplit access.log '/^Error/' '{*}'")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different patterns collide", func(t *testing.T) {
		a := shellshape.Normalize("csplit file.txt '/^Chapter/'")
		b := shellshape.Normalize("csplit file.txt '/^Section/'")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	// SAFETY TEST
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("csplit literal-arg 100")
		subshell := shellshape.Normalize("csplit $(dangerous-command) 100")
		if benign == subshell {
			t.Error("subshell must produce different shape than literal")
		}
	})
