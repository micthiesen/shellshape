...
func TestAb(t *testing.T) {
	...
	t.Run("different URLs collide", func(t *testing.T) {
		a := shellshape.Normalize("ab -n 100 https://api.example.com/users")
		b := shellshape.Normalize("ab -n 100 https://api.other.com/items")
		if a != b {
	...
	t.Run("different auth collide", func(t *testing.T) {
		a := shellshape.Normalize("ab -A alice:pass1 https://example.com/")
		b := shellshape.Normalize("ab -A bob:pass2 https://example.com/")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("ab https://example.com/")
		subshell := shellshape.Normalize("ab $(dangerous-command)")
		if benign == subshell {
...
func TestWrk(t *testing.T) {
	...
	// COLLISION TESTS
	t.Run("different URLs collide", func(t *testing.T) {
		a := shellshape.Normalize("wrk -t4 -c100 -d30s https://api.example.com/users")
		b := shellshape.Normalize("wrk -t4 -c100 -d30s https://api.other.com/items")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	// SAFETY TEST
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("wrk https://example.com/")
		subshell := shellshape.Normalize("wrk $(dangerous-command)")
		if benign == subshell {
			t.Error("subshell must produce different shape than literal")
		}
	})
...
func TestHey(t *testing.T) {
	...
	// COLLISION TESTS
	t.Run("different URLs collide", func(t *testing.T) {
		a := shellshape.Normalize("hey -n 200 https://api.example.com/users")
		b := shellshape.Normalize("hey -n 200 https://api.other.com/items")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different body data collide", func(t *testing.T) {
		a := shellshape.Normalize("hey -d '{\"name\":\"alice\"}' https://example.com/")
		b := shellshape.Normalize("hey -d '{\"name\":\"bob\"}' https://example.com/")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	// SAFETY TEST
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("hey https://example.com/")
		subshell := shellshape.Normalize("hey $(dangerous-command)")
		if benign == subshell {
			t.Error("subshell must produce different shape than literal")
		}
	})
