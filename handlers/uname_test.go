...
func TestUname(t *testing.T) {
	...
	t.Run("different positionals collide", func(t *testing.T) {
		a := shellshape.Normalize("uname foo")
		b := shellshape.Normalize("uname bar")
		if a != b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("uname foo")
		subshell := shellshape.Normalize("uname $(dangerous-command)")
		if benign == subshell {
