...
func TestMvn(t *testing.T) {
	...
	t.Run("different property values collide", func(t *testing.T) {
		a := shellshape.Normalize("mvn clean package -Dapp.version=1.0.0")
		b := shellshape.Normalize("mvn clean package -Dapp.version=2.5.3")
		if a != b {
	...
	t.Run("different profiles collide", func(t *testing.T) {
		a := shellshape.Normalize("mvn clean package -Pdev")
		b := shellshape.Normalize("mvn clean package -Pproduction")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different pom paths collide", func(t *testing.T) {
		a := shellshape.Normalize("mvn -f /home/alice/project/pom.xml install")
		b := shellshape.Normalize("mvn -f /home/bob/project/pom.xml install")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("mvn clean package")
		subshell := shellshape.Normalize("mvn $(dangerous-command)")
		if benign == subshell {
