...
func TestHtpasswd(t *testing.T) {
	...
	t.Run("different users collide", func(t *testing.T) {
		a := shellshape.Normalize("htpasswd /etc/htpasswd alice")
		b := shellshape.Normalize("htpasswd /etc/htpasswd bob")
		if a != b {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("htpasswd -c /var/www/.htpasswd admin")
		b := shellshape.Normalize("htpasswd -c /etc/apache2/.htpasswd admin")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("htpasswd /etc/htpasswd admin")
		subshell := shellshape.Normalize("htpasswd $(dangerous-command)")
		if benign == subshell {
