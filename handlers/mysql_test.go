...
func TestMysql(t *testing.T) {
	...
	t.Run("different databases collide", func(t *testing.T) {
		a := shellshape.Normalize("mysql -u root -p production")
		b := shellshape.Normalize("mysql -u root -p staging")
		if a != b {
	...
	t.Run("different hosts collide", func(t *testing.T) {
		a := shellshape.Normalize("mysql -h 10.0.0.1 -u root mydb")
		b := shellshape.Normalize("mysql -h 10.0.0.2 -u admin mydb")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("mysql mydb")
		subshell := shellshape.Normalize("mysql $(dangerous-command)")
		if benign == subshell {
