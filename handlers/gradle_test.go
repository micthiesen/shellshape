...
func TestGradle(t *testing.T) {
	...
	t.Run("different excluded tasks collide", func(t *testing.T) {
		a := shellshape.Normalize("gradle build -x test")
		b := shellshape.Normalize("gradle build -x lint")
		if a != b {
	...
	t.Run("different project dirs collide", func(t *testing.T) {
		a := shellshape.Normalize("gradle -p /home/alice/project build")
		b := shellshape.Normalize("gradle -p /home/bob/project build")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different system props collide", func(t *testing.T) {
		a := shellshape.Normalize("gradle -Dfoo=bar build")
		b := shellshape.Normalize("gradle -Dbaz=qux build")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("gradle build")
		subshell := shellshape.Normalize("gradle $(dangerous-command)")
		if benign == subshell {
