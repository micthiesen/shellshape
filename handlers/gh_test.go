...
func TestGh(t *testing.T) {
	...
	t.Run("different titles collide", func(t *testing.T) {
		a := shellshape.Normalize("gh pr create --title 'Fix login bug' --body 'Fixes the auth issue'")
		b := shellshape.Normalize("gh pr create --title 'Add feature X' --body 'New capability'")
		if a != b {
	...
	t.Run("different PR numbers collide", func(t *testing.T) {
		a := shellshape.Normalize("gh pr view 123")
		b := shellshape.Normalize("gh pr view 456")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different repos collide", func(t *testing.T) {
		a := shellshape.Normalize("gh pr list -R owner/repo1")
		b := shellshape.Normalize("gh pr list -R owner/repo2")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("gh pr view 123")
		subshell := shellshape.Normalize("gh pr view $(get-pr-number)")
		if benign == subshell {
