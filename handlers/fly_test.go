...
func TestFly(t *testing.T) {
	...
	t.Run("different app names collide", func(t *testing.T) {
		a := shellshape.Normalize("fly deploy --app my-app-1")
		b := shellshape.Normalize("fly deploy --app my-app-2")
		if a != b {
	...
	t.Run("different env values collide", func(t *testing.T) {
		a := shellshape.Normalize("fly deploy -e FOO=bar")
		b := shellshape.Normalize("fly deploy -e BAZ=qux")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different regions collide", func(t *testing.T) {
		a := shellshape.Normalize("fly deploy --region lax")
		b := shellshape.Normalize("fly deploy --region ord")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("fly deploy --app my-app")
		subshell := shellshape.Normalize("fly deploy --app $(get-app-name)")
		if benign == subshell {
