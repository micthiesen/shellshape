...
func TestFnm(t *testing.T) {
	...
	t.Run("different versions collide", func(t *testing.T) {
		a := shellshape.Normalize("fnm install 18")
		b := shellshape.Normalize("fnm install 20.11.0")
		if a != b {
	...
	t.Run("different fnm-dir paths collide", func(t *testing.T) {
		a := shellshape.Normalize("fnm install --fnm-dir /home/alice/.fnm 18")
		b := shellshape.Normalize("fnm install --fnm-dir /home/bob/.fnm 20")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("fnm install 18")
		subshell := shellshape.Normalize("fnm install $(dangerous-command)")
		if benign == subshell {
