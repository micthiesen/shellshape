...
func TestPython(t *testing.T) {
	...
	t.Run("different scripts collide", func(t *testing.T) {
		a := shellshape.Normalize("python server.py --port 3000")
		b := shellshape.Normalize("python worker.py --port 8080")
		if a != b {
	...
	t.Run("different module args collide", func(t *testing.T) {
		a := shellshape.Normalize("python -m pytest tests/unit")
		b := shellshape.Normalize("python -m pytest tests/integration")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different code collides", func(t *testing.T) {
		a := shellshape.Normalize("python -c 'print(1)'")
		b := shellshape.Normalize("python -c 'import os'")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("python script.py")
		subshell := shellshape.Normalize("python $(dangerous-command)")
		if benign == subshell {
