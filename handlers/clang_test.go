...
func TestClang(t *testing.T) {
	...
	t.Run("different source files collide", func(t *testing.T) {
		a := shellshape.Normalize("clang foo.c -o app")
		b := shellshape.Normalize("clang bar.c -o app")
		if a != b {
	...
	t.Run("different define values collide", func(t *testing.T) {
		a := shellshape.Normalize("clang -DFOO=1 main.c")
		b := shellshape.Normalize("clang -DBAR=2 main.c")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different include paths collide", func(t *testing.T) {
		a := shellshape.Normalize("clang -I/usr/include main.c")
		b := shellshape.Normalize("clang -I/opt/local/include main.c")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different libraries collide", func(t *testing.T) {
		a := shellshape.Normalize("clang main.c -lm")
		b := shellshape.Normalize("clang main.c -lpthread")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("clang literal-arg")
		subshell := shellshape.Normalize("clang $(dangerous-command)")
		if benign == subshell {
