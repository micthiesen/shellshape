...
func TestDd(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := shellshape.Normalize("dd if=/dev/sda of=/dev/sdb bs=4096")
		b := shellshape.Normalize("dd if=/tmp/image.iso of=/dev/disk3 bs=4096")
		if a != b {
	...
	t.Run("different sizes collide", func(t *testing.T) {
		a := shellshape.Normalize("dd if=/dev/zero of=file.dat bs=1024 count=100")
		b := shellshape.Normalize("dd if=/dev/zero of=file.dat bs=4M count=5000")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("dd if=/dev/zero of=output.bin bs=1024")
		subshell := shellshape.Normalize("dd if=$(dangerous-command) of=output.bin bs=1024")
		if benign == subshell {
