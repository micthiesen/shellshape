...
func TestCat(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("cat /etc/hosts")
		b := shellshape.Normalize("cat /var/log/syslog")
		c := shellshape.Normalize("cat ~/.bashrc")
		if a != b || b != c {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("cat literal-arg")
		subshell := shellshape.Normalize("cat $(dangerous-command)")
		if benign == subshell {
...
func TestRev(t *testing.T) {
	...
	// COLLISION TESTS: different file paths → same shape
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("rev /etc/hosts")
		b := shellshape.Normalize("rev /var/log/syslog")
		c := shellshape.Normalize("rev ~/.bashrc")
		if a != b || b != c {
			t.Errorf("expected all same: %q, %q, %q", a, b, c)
		}
	})
	...
	// SAFETY TEST: subshell must not collapse
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("rev literal-arg")
		subshell := shellshape.Normalize("rev $(dangerous-command)")
		if benign == subshell {
			t.Error("subshell must produce different shape than literal")
		}
	})
