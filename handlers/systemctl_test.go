...
func TestSystemctl(t *testing.T) {
	...
	t.Run("different unit names collide", func(t *testing.T) {
		a := shellshape.Normalize("systemctl restart nginx.service")
		b := shellshape.Normalize("systemctl restart postgresql.service")
		if a != b {
	...
	t.Run("different property values collide", func(t *testing.T) {
		a := shellshape.Normalize("systemctl show -p ActiveState nginx")
		b := shellshape.Normalize("systemctl show -p SubState sshd")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different hosts collide", func(t *testing.T) {
		a := shellshape.Normalize("systemctl -H root@server1 status nginx")
		b := shellshape.Normalize("systemctl -H admin@server2 status sshd")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("systemctl start literal-arg")
		subshell := shellshape.Normalize("systemctl start $(dangerous-command)")
		if benign == subshell {
