...
func TestHtop(t *testing.T) {
	...
	t.Run("different users collide", func(t *testing.T) {
		a := shellshape.Normalize("htop -u root")
		b := shellshape.Normalize("htop -u nobody")
		if a != b {
	...
	t.Run("different pids collide", func(t *testing.T) {
		a := shellshape.Normalize("htop -p 1234")
		b := shellshape.Normalize("htop -p 9999,8888")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different delays collide", func(t *testing.T) {
		a := shellshape.Normalize("htop -d 5")
		b := shellshape.Normalize("htop -d 50")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("htop -u root")
		subshell := shellshape.Normalize("htop -u $(dangerous-command)")
		if benign == subshell {
