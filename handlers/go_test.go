...
func TestGo(t *testing.T) {
	...
	t.Run("different test names collide", func(t *testing.T) {
		a := shellshape.Normalize("go test -run TestFoo -v ./...")
		b := shellshape.Normalize("go test -run TestBar -v ./...")
		if a != b {
	...
	t.Run("different build outputs collide", func(t *testing.T) {
		a := shellshape.Normalize("go build -o /tmp/app1 .")
		b := shellshape.Normalize("go build -o /tmp/app2 .")
		if a != b {
			t.Errorf("expected same shape, got %q vs %q", a, b)
		}
	})
	...
	t.Run("different timeout values collide", func(t *testing.T) {
		a := shellshape.Normalize("go test -timeout 30s ./...")
		b := shellshape.Normalize("go test -timeout 5m ./...")
		if a != b {
			t.Errorf("expected same shape, got %q vs %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("go test ./pkg/foo")
		subshell := shellshape.Normalize("go test $(dangerous-command)")
		if benign == subshell {
