...
func TestPsql(t *testing.T) {
	...
	t.Run("different databases collide", func(t *testing.T) {
		a := shellshape.Normalize("psql -h localhost production")
		b := shellshape.Normalize("psql -h localhost staging")
		if a != b {
	...
	t.Run("different queries collide", func(t *testing.T) {
		a := shellshape.Normalize("psql -c 'SELECT * FROM users' mydb")
		b := shellshape.Normalize("psql -c 'DELETE FROM orders' mydb")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("psql mydb")
		subshell := shellshape.Normalize("psql $(echo mydb)")
		if benign == subshell {
