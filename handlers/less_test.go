...
func TestLess(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("less /etc/hosts")
		b := shellshape.Normalize("less /var/log/syslog")
		c := shellshape.Normalize("less ~/.bashrc")
		if a != b || b != c {
	...
	t.Run("different patterns collide", func(t *testing.T) {
		a := shellshape.Normalize("less -p 'error' file.txt")
		b := shellshape.Normalize("less -p 'warning' file.txt")
		if a != b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("less literal-arg")
		subshell := shellshape.Normalize("less $(dangerous-command)")
		if benign == subshell {
