...
func TestNc(t *testing.T) {
	...
	t.Run("different hosts collide", func(t *testing.T) {
		a := shellshape.Normalize("nc google.com 443")
		b := shellshape.Normalize("nc example.org 443")
		if a != b {
	...
	t.Run("different ports collide", func(t *testing.T) {
		a := shellshape.Normalize("nc host.example.com 80")
		b := shellshape.Normalize("nc host.example.com 8080")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("nc host.example.com 80")
		subshell := shellshape.Normalize("nc $(dangerous-command) 80")
		if benign == subshell {
