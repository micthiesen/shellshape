...
func TestCurl(t *testing.T) {
	...
	t.Run("different URLs collide", func(t *testing.T) {
		a := shellshape.Normalize("curl -s https://api.example.com/users")
		b := shellshape.Normalize("curl -s https://api.other.com/items")
		if a != b {
	...
	t.Run("different data collide", func(t *testing.T) {
		a := shellshape.Normalize("curl -d 'user=alice' https://example.com")
		b := shellshape.Normalize("curl -d 'user=bob' https://example.com")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different headers collide", func(t *testing.T) {
		a := shellshape.Normalize("curl -H 'Authorization: Bearer abc' https://example.com")
		b := shellshape.Normalize("curl -H 'Authorization: Bearer xyz' https://example.com")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("curl https://example.com")
		subshell := shellshape.Normalize("curl $(dangerous-command)")
		if benign == subshell {
