...
func TestNice(t *testing.T) {
	...
	t.Run("different priorities collide", func(t *testing.T) {
		a := shellshape.Normalize("nice -n 10 make")
		b := shellshape.Normalize("nice -n 15 make")
		c := shellshape.Normalize("nice -n 5 make")
		if a != b || b != c {
	...
	t.Run("different command args collide", func(t *testing.T) {
		a := shellshape.Normalize("nice gcc -o main foo.c")
		b := shellshape.Normalize("nice gcc -o main bar.c")
		if a != b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("nice make target")
		subshell := shellshape.Normalize("nice $(dangerous-command)")
		if benign == subshell {
