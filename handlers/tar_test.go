...
func TestTar(t *testing.T) {
	...
	t.Run("different archives collide", func(t *testing.T) {
		a := shellshape.Normalize("tar -xzf project-v1.tar.gz")
		b := shellshape.Normalize("tar -xzf backup-2024.tar.gz")
		if a != b {
	...
	t.Run("different directories collide", func(t *testing.T) {
		a := shellshape.Normalize("tar -czf archive.tar.gz /home/alice/docs")
		b := shellshape.Normalize("tar -czf archive.tar.gz /var/log/nginx")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("tar -xf archive.tar")
		subshell := shellshape.Normalize("tar -xf $(dangerous-command)")
		if benign == subshell {
