...
func TestPaste(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("paste /tmp/a.txt /tmp/b.txt")
		b := shellshape.Normalize("paste /var/x.csv /var/y.csv")
		if a != b {
	...
	t.Run("different delimiters collide", func(t *testing.T) {
		a := shellshape.Normalize("paste -d : file1.txt file2.txt")
		b := shellshape.Normalize("paste -d , file1.txt file2.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("paste literal-arg")
		subshell := shellshape.Normalize("paste $(dangerous-command)")
		if benign == subshell {
