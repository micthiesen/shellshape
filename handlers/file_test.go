...
func TestFile(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("file -i /tmp/photo.png")
		b := shellshape.Normalize("file -i /var/data/report.csv")
		if a != b {
	...
	t.Run("different separators collide", func(t *testing.T) {
		a := shellshape.Normalize("file -F : foo.txt")
		b := shellshape.Normalize("file -F @ bar.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("file literal-arg")
		subshell := shellshape.Normalize("file $(dangerous-command)")
		if benign == subshell {
