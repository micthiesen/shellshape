...
func TestUptime(t *testing.T) {
	...
	t.Run("different redirects collide", func(t *testing.T) {
		a := shellshape.Normalize("uptime > /tmp/a.txt")
		b := shellshape.Normalize("uptime > /var/log/b.log")
		if a != b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("uptime somefile")
		subshell := shellshape.Normalize("uptime $(dangerous-command)")
		if benign == subshell {
