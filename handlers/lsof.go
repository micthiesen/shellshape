...
func init() {
	shellshape.Register("lsof", handleLsof)
}
...
func handleLsof(_ string, tokens []string) []string {
	...
	for i < len(args) {
		...
		if ph, ok := valueFlags[tok]; ok {
			result, i = shellshape.ConsumeFlagArg(tok, args, i, result, ph)
			continue
		}
		...
		if shellshape.IsFlagToken(tok) || tok == "+w" || tok == "+E" || tok == "+L" {
			result = append(result, tok)
			i++
			continue
		}
...
func isNetworkSpec(tok string) bool {
	if shellshape.IsFlagToken(tok) || shellshape.IsSubshellToken(tok) {
		return false
	}
