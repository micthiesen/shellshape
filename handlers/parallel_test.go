...
func TestParallel(t *testing.T) {
	...
	t.Run("different args collide", func(t *testing.T) {
		a := shellshape.Normalize("parallel gzip ::: fileA.dat fileB.dat")
		b := shellshape.Normalize("parallel gzip ::: x.log y.log z.log")
		if a != b {
	...
	t.Run("different hosts collide", func(t *testing.T) {
		a := shellshape.Normalize("parallel -S host1,host2 uptime ::: a")
		b := shellshape.Normalize("parallel -S server3,server4 uptime ::: a")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("parallel echo ::: literal-arg")
		subshell := shellshape.Normalize("parallel echo ::: $(dangerous-command)")
		if benign == subshell {
