...
func TestNft(t *testing.T) {
	...
	t.Run("different table names collide", func(t *testing.T) {
		a := shellshape.Normalize("nft add table inet filter")
		b := shellshape.Normalize("nft add table inet mychain")
		if a != b {
	...
	t.Run("different rule bodies collide", func(t *testing.T) {
		a := shellshape.Normalize("nft add rule inet filter input tcp dport 22 accept")
		b := shellshape.Normalize("nft add rule inet filter input udp sport 53 drop")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("nft add table inet mytable")
		subshell := shellshape.Normalize("nft add table inet $(dangerous-command)")
		if benign == subshell {
