...
func TestNslookup(t *testing.T) {
	...
	t.Run("different domains collide", func(t *testing.T) {
		a := shellshape.Normalize("nslookup example.com")
		b := shellshape.Normalize("nslookup google.com")
		if a != b {
	...
	t.Run("different servers collide", func(t *testing.T) {
		a := shellshape.Normalize("nslookup example.com 8.8.8.8")
		b := shellshape.Normalize("nslookup example.com 1.1.1.1")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different timeouts collide", func(t *testing.T) {
		a := shellshape.Normalize("nslookup -timeout=5 example.com")
		b := shellshape.Normalize("nslookup -timeout=30 google.com")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("nslookup literal-arg")
		subshell := shellshape.Normalize("nslookup $(dangerous-command)")
		if benign == subshell {
