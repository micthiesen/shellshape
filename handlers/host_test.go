...
func TestHost(t *testing.T) {
	...
	t.Run("different hostnames collide", func(t *testing.T) {
		a := shellshape.Normalize("host example.com")
		b := shellshape.Normalize("host google.com")
		if a != b {
	...
	t.Run("different servers collide", func(t *testing.T) {
		a := shellshape.Normalize("host example.com 8.8.8.8")
		b := shellshape.Normalize("host example.com 1.1.1.1")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different numeric values collide", func(t *testing.T) {
		a := shellshape.Normalize("host -R 3 -W 5 example.com")
		b := shellshape.Normalize("host -R 10 -W 30 google.com")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("host literal-arg")
		subshell := shellshape.Normalize("host $(dangerous-command)")
		if benign == subshell {
