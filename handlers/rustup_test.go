...
func TestRustup(t *testing.T) {
	...
	t.Run("different toolchains collide", func(t *testing.T) {
		a := shellshape.Normalize("rustup install stable")
		b := shellshape.Normalize("rustup install nightly-2024-01-15")
		if a != b {
	...
	t.Run("different targets collide", func(t *testing.T) {
		a := shellshape.Normalize("rustup target add wasm32-unknown-unknown")
		b := shellshape.Normalize("rustup target add x86_64-pc-windows-msvc")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("rustup install stable")
		subshell := shellshape.Normalize("rustup install $(dangerous-command)")
		if benign == subshell {
