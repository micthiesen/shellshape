...
func TestEnv(t *testing.T) {
	...
	t.Run("different assignments collide", func(t *testing.T) {
		a := shellshape.Normalize("env FOO=bar bash")
		b := shellshape.Normalize("env DATABASE_URL=postgres://localhost/db bash")
		if a != b {
	...
	t.Run("different unset vars collide", func(t *testing.T) {
		a := shellshape.Normalize("env -u HOME bash")
		b := shellshape.Normalize("env -u PATH bash")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("env FOO=bar bash")
		subshell := shellshape.Normalize("env $(dangerous-command)")
		if benign == subshell {
