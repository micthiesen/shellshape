...
func TestXargs(t *testing.T) {
	...
	t.Run("different utility args collide", func(t *testing.T) {
		a := shellshape.Normalize("xargs -I {} cp {} /backup/dir1")
		b := shellshape.Normalize("xargs -I {} cp {} /backup/dir2")
		if a != b {
	...
	t.Run("different numeric args collide", func(t *testing.T) {
		a := shellshape.Normalize("xargs -n 1 rm")
		b := shellshape.Normalize("xargs -n 10 rm")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("xargs echo hello")
		subshell := shellshape.Normalize("xargs echo $(dangerous-command)")
		if benign == subshell {
