...
func TestDig(t *testing.T) {
	...
	t.Run("different domains collide", func(t *testing.T) {
		a := shellshape.Normalize("dig example.com MX")
		b := shellshape.Normalize("dig google.com MX")
		if a != b {
	...
	t.Run("different servers collide", func(t *testing.T) {
		a := shellshape.Normalize("dig @8.8.8.8 example.com")
		b := shellshape.Normalize("dig @1.1.1.1 google.com")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("dig example.com")
		subshell := shellshape.Normalize("dig $(dangerous-command)")
		if benign == subshell {
