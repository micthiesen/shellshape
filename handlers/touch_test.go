...
func TestTouch(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("touch /home/user/foo.txt")
		b := shellshape.Normalize("touch /var/log/bar.log")
		if a != b {
	...
	t.Run("different timestamps collide", func(t *testing.T) {
		a := shellshape.Normalize("touch -t 202301011200.00 file.txt")
		b := shellshape.Normalize("touch -t 199912312359.59 other.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("touch literal-arg")
		subshell := shellshape.Normalize("touch $(dangerous-command)")
		if benign == subshell {
