...
func TestSu(t *testing.T) {
	...
	t.Run("different users collide", func(t *testing.T) {
		a := shellshape.Normalize("su alice")
		b := shellshape.Normalize("su bob")
		if a != b {
	...
	t.Run("different commands collide", func(t *testing.T) {
		a := shellshape.Normalize("su -c 'whoami' root")
		b := shellshape.Normalize("su -c 'id' root")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("su literal-arg")
		subshell := shellshape.Normalize("su $(dangerous-command)")
		if benign == subshell {
