...
func TestBun(t *testing.T) {
	...
	t.Run("different file paths collide", func(t *testing.T) {
		a := shellshape.Normalize("bun run ./src/server.ts")
		b := shellshape.Normalize("bun run ./src/client.ts")
		if a != b {
	...
	t.Run("different timeout values collide", func(t *testing.T) {
		a := shellshape.Normalize("bun test --timeout 5000")
		b := shellshape.Normalize("bun test --timeout 10000")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different env-file paths collide", func(t *testing.T) {
		a := shellshape.Normalize("bun run --env-file .env.local dev")
		b := shellshape.Normalize("bun run --env-file .env.production dev")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("bun run script-name")
