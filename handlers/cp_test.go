...
func TestCp(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("cp /etc/hosts /tmp/hosts")
		b := shellshape.Normalize("cp /var/log/syslog /backup/syslog")
		c := shellshape.Normalize("cp ~/.bashrc /tmp/bashrc")
		if a != b || b != c {
	...
	t.Run("different flags same paths collide", func(t *testing.T) {
		a := shellshape.Normalize("cp -r /home/alice/ /backup/")
		b := shellshape.Normalize("cp -r /home/bob/ /archive/")
		if a != b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("cp literal-arg")
		subshell := shellshape.Normalize("cp $(dangerous-command)")
		if benign == subshell {
