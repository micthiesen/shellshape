...
func TestMkdir(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := shellshape.Normalize("mkdir -p /home/alice/project")
		b := shellshape.Normalize("mkdir -p /var/log/myapp")
		if a != b {
	...
	t.Run("different modes collide", func(t *testing.T) {
		a := shellshape.Normalize("mkdir -m 755 /tmp/a")
		b := shellshape.Normalize("mkdir -m 700 /tmp/b")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("mkdir literal-arg")
		subshell := shellshape.Normalize("mkdir $(dangerous-command)")
		if benign == subshell {
