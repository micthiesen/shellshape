...
func TestTsx(t *testing.T) {
	...
	t.Run("different scripts collide", func(t *testing.T) {
		a := shellshape.Normalize("tsx src/server.ts")
		b := shellshape.Normalize("tsx src/worker.ts")
		if a != b {
	...
	t.Run("different script args collide", func(t *testing.T) {
		a := shellshape.Normalize("tsx app.ts --port 3000 --host 0.0.0.0")
		b := shellshape.Normalize("tsx app.ts --port 8080 --host localhost")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different modules collide", func(t *testing.T) {
		a := shellshape.Normalize("tsx -r dotenv/config app.ts")
		b := shellshape.Normalize("tsx -r tsconfig-paths/register app.ts")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("tsx app.ts")
		subshell := shellshape.Normalize("tsx $(dangerous-command)")
		if benign == subshell {
