...
func TestSnap(t *testing.T) {
	...
	t.Run("different find queries collide", func(t *testing.T) {
		a := shellshape.Normalize("snap find postgres")
		b := shellshape.Normalize("snap find redis")
		if a != b {
	...
	t.Run("different change IDs collide", func(t *testing.T) {
		a := shellshape.Normalize("snap change 100")
		b := shellshape.Normalize("snap change 999")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("snap install literal-arg")
		subshell := shellshape.Normalize("snap install $(dangerous-command)")
		if benign == subshell {
