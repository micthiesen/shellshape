...
func TestJournalctl(t *testing.T) {
	...
	t.Run("different units collide", func(t *testing.T) {
		a := shellshape.Normalize("journalctl -u nginx")
		b := shellshape.Normalize("journalctl -u sshd")
		if a != b {
	...
	t.Run("different field matches collide", func(t *testing.T) {
		a := shellshape.Normalize("journalctl _PID=123")
		b := shellshape.Normalize("journalctl _COMM=sshd")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different timestamps collide", func(t *testing.T) {
		a := shellshape.Normalize("journalctl --since '2024-01-01' --until '2024-01-02'")
		b := shellshape.Normalize("journalctl --since yesterday --until today")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("journalctl -u nginx")
		subshell := shellshape.Normalize("journalctl -u $(dangerous-command)")
		if benign == subshell {
