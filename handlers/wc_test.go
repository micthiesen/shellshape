...
func TestWc(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("wc -l /var/log/syslog")
		b := shellshape.Normalize("wc -l /tmp/output.txt")
		if a != b {
	...
	t.Run("different multi files collide", func(t *testing.T) {
		a := shellshape.Normalize("wc -w report1.txt report2.txt")
		b := shellshape.Normalize("wc -w data.csv results.csv")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("wc file.txt")
		subshell := shellshape.Normalize("wc $(dangerous-command)")
		if benign == subshell {
