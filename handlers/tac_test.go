...
func TestTac(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("tac /etc/hosts")
		b := shellshape.Normalize("tac /var/log/syslog")
		c := shellshape.Normalize("tac ~/.bashrc")
		if a != b || b != c {
	...
	t.Run("different separators collide", func(t *testing.T) {
		a := shellshape.Normalize("tac -s '---' file.txt")
		b := shellshape.Normalize("tac -s '===' file.txt")
		if a != b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("tac literal-arg")
		subshell := shellshape.Normalize("tac $(dangerous-command)")
		if benign == subshell {
