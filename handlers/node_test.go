...
func TestNode(t *testing.T) {
	...
	t.Run("different scripts collide", func(t *testing.T) {
		a := shellshape.Normalize("node server.js --port 3000")
		b := shellshape.Normalize("node worker.js --port 8080")
		if a != b {
	...
	t.Run("different eval code collides", func(t *testing.T) {
		a := shellshape.Normalize("node -e 'console.log(1)'")
		b := shellshape.Normalize("node -e 'process.exit(0)'")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("node app.js")
		subshell := shellshape.Normalize("node $(dangerous-command)")
		if benign == subshell {
