...
func TestFdisk(t *testing.T) {
	...
	t.Run("different devices collide", func(t *testing.T) {
		a := shellshape.Normalize("fdisk /dev/sda")
		b := shellshape.Normalize("fdisk /dev/nvme0n1")
		if a != b {
	...
	t.Run("different numeric values collide", func(t *testing.T) {
		a := shellshape.Normalize("fdisk -b 512 /dev/sda")
		b := shellshape.Normalize("fdisk -b 4096 /dev/sdb")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("fdisk /dev/sda")
		subshell := shellshape.Normalize("fdisk $(echo /dev/sda)")
		if benign == subshell {
