...
func TestStrings(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("strings -n 8 /usr/bin/ls")
		b := shellshape.Normalize("strings -n 4 /usr/bin/cat")
		if a != b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("strings literal-arg")
		subshell := shellshape.Normalize("strings $(dangerous-command)")
		if benign == subshell {
