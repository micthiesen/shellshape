...
func TestRsync(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := shellshape.Normalize("rsync -avz /home/alice/docs/ /mnt/backup/alice/")
		b := shellshape.Normalize("rsync -avz /home/bob/photos/ /mnt/backup/bob/")
		if a != b {
	...
	t.Run("different exclude patterns collide", func(t *testing.T) {
		a := shellshape.Normalize("rsync -avz --exclude '*.log' src/ dest/")
		b := shellshape.Normalize("rsync -avz --exclude '*.tmp' src/ dest/")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("rsync -avz /data/ /backup/")
		subshell := shellshape.Normalize("rsync -avz $(dangerous-command) /backup/")
		if benign == subshell {
