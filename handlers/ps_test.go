...
func TestPs(t *testing.T) {
	...
	t.Run("different users collide", func(t *testing.T) {
		a := shellshape.Normalize("ps -u root")
		b := shellshape.Normalize("ps -u nobody")
		if a != b {
	...
	t.Run("different pids collide", func(t *testing.T) {
		a := shellshape.Normalize("ps -p 1234")
		b := shellshape.Normalize("ps -p 9876")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("ps -p 1234")
		subshell := shellshape.Normalize("ps -p $(pgrep nginx)")
		if benign == subshell {
