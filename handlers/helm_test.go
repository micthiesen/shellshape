...
func TestHelm(t *testing.T) {
	...
	t.Run("different set values collide", func(t *testing.T) {
		a := shellshape.Normalize("helm install my-release bitnami/nginx --set image.tag=v1.0")
		b := shellshape.Normalize("helm install my-release bitnami/nginx --set image.tag=v2.0")
		if a != b {
	...
	t.Run("different namespace values collide", func(t *testing.T) {
		a := shellshape.Normalize("helm install my-release bitnami/nginx -n staging")
		b := shellshape.Normalize("helm install my-release bitnami/nginx -n production")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different values files collide", func(t *testing.T) {
		a := shellshape.Normalize("helm install my-release bitnami/nginx -f values-dev.yaml")
		b := shellshape.Normalize("helm install my-release bitnami/nginx -f values-prod.yaml")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("helm install my-release bitnami/nginx")
		subshell := shellshape.Normalize("helm install $(dangerous-command) bitnami/nginx")
		if benign == subshell {
