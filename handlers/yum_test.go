...
func TestYum(t *testing.T) {
	...
	t.Run("different search terms collide", func(t *testing.T) {
		a := shellshape.Normalize("yum search postgres")
		b := shellshape.Normalize("yum search redis")
		if a != b {
	...
	t.Run("different search terms collide dnf", func(t *testing.T) {
		a := shellshape.Normalize("dnf search webserver")
		b := shellshape.Normalize("dnf search database")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("yum install literal-arg")
		subshell := shellshape.Normalize("yum install $(dangerous-command)")
		if benign == subshell {
	...
	t.Run("subshell not collapsed dnf", func(t *testing.T) {
		benign := shellshape.Normalize("dnf install literal-arg")
		subshell := shellshape.Normalize("dnf install $(dangerous-command)")
		if benign == subshell {
			t.Error("subshell must produce different shape than literal")
		}
	})
