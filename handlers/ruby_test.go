...
func TestRuby(t *testing.T) {
	...
	t.Run("different scripts collide", func(t *testing.T) {
		a := shellshape.Normalize("ruby server.rb --port 3000")
		b := shellshape.Normalize("ruby worker.rb --port 8080")
		if a != b {
	...
	t.Run("different eval code collides", func(t *testing.T) {
		a := shellshape.Normalize("ruby -e 'puts 1'")
		b := shellshape.Normalize("ruby -e 'puts 2'")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("ruby script.rb")
		subshell := shellshape.Normalize("ruby $(dangerous-command)")
		if benign == subshell {
