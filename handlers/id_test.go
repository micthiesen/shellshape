...
func TestId(t *testing.T) {
	...
	t.Run("different users collide", func(t *testing.T) {
		a := shellshape.Normalize("id alice")
		b := shellshape.Normalize("id bob")
		c := shellshape.Normalize("id root")
		if a != b || b != c {
	...
	t.Run("different uids collide", func(t *testing.T) {
		a := shellshape.Normalize("id 0")
		b := shellshape.Normalize("id 501")
		c := shellshape.Normalize("id 1000")
		if a != b || b != c {
			t.Errorf("expected all same: %q, %q, %q", a, b, c)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("id bob")
		subshell := shellshape.Normalize("id $(dangerous-command)")
		if benign == subshell {
