...
func TestLdd(t *testing.T) {
	...
	t.Run("different binaries collide", func(t *testing.T) {
		a := shellshape.Normalize("ldd /usr/bin/ls")
		b := shellshape.Normalize("ldd /usr/bin/cat")
		if a != b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("ldd literal-arg")
		subshell := shellshape.Normalize("ldd $(dangerous-command)")
		if benign == subshell {
...
func TestOtool(t *testing.T) {
	...
	// Collision tests
	t.Run("different binaries collide", func(t *testing.T) {
		a := shellshape.Normalize("otool -L /usr/lib/libfoo.dylib")
		b := shellshape.Normalize("otool -L /Applications/MyApp.app/Contents/MacOS/MyApp")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different arch values collide", func(t *testing.T) {
		a := shellshape.Normalize("otool -arch x86_64 -L myapp")
		b := shellshape.Normalize("otool -arch arm64 -L myapp")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	// Subshell safety test
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("otool -L literal-arg")
		subshell := shellshape.Normalize("otool -L $(dangerous-command)")
		if benign == subshell {
			t.Error("subshell must produce different shape than literal")
		}
	})
