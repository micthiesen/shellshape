...
func TestBiome(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := shellshape.Normalize("biome check --write src/components/")
		b := shellshape.Normalize("biome check --write src/utils/")
		if a != b {
	...
	t.Run("different flag values collide", func(t *testing.T) {
		a := shellshape.Normalize("biome lint --colors force src/")
		b := shellshape.Normalize("biome lint --colors off src/")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("biome check src/")
		subshell := shellshape.Normalize("biome check $(dangerous-command)")
		if benign == subshell {
