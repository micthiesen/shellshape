...
func TestCal(t *testing.T) {
	...
	t.Run("different years collide", func(t *testing.T) {
		a := shellshape.Normalize("cal 2024")
		b := shellshape.Normalize("cal 1999")
		c := shellshape.Normalize("cal 2030")
		if a != b || b != c {
	...
	t.Run("different month-year pairs collide", func(t *testing.T) {
		a := shellshape.Normalize("cal 1 2024")
		b := shellshape.Normalize("cal 12 1999")
		if a != b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("cal 2024")
		subshell := shellshape.Normalize("cal $(date +%Y)")
		if benign == subshell {
