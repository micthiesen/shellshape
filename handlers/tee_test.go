...
func TestTee(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := shellshape.Normalize("tee -a /var/log/app.log")
		b := shellshape.Normalize("tee -a /tmp/debug.txt")
		if a != b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("tee literal-arg")
		subshell := shellshape.Normalize("tee $(dangerous-command)")
		if benign == subshell {
