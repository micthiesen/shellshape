...
func TestMkfs(t *testing.T) {
	...
	t.Run("different devices collide", func(t *testing.T) {
		a := shellshape.Normalize("mkfs.ext4 /dev/sda1")
		b := shellshape.Normalize("mkfs.ext4 /dev/nvme0n1p2")
		if a != b {
	...
	t.Run("different labels collide", func(t *testing.T) {
		a := shellshape.Normalize("mkfs.ext4 -L my-data /dev/sda1")
		b := shellshape.Normalize("mkfs.ext4 -L backups /dev/sda1")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different block sizes collide", func(t *testing.T) {
		a := shellshape.Normalize("mkfs.ext4 -b 1024 /dev/sda1")
		b := shellshape.Normalize("mkfs.ext4 -b 4096 /dev/sda1")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("mkfs.ext4 /dev/sda1")
		subshell := shellshape.Normalize("mkfs.ext4 $(dangerous-command)")
		if benign == subshell {
