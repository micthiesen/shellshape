...
func TestEnvsubst(t *testing.T) {
	...
	t.Run("different shell-formats collide", func(t *testing.T) {
		a := shellshape.Normalize("envsubst '$HOME $USER'")
		b := shellshape.Normalize("envsubst '$PATH'")
		c := shellshape.Normalize("envsubst '${DATABASE_URL}'")
		if a != b || b != c {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("envsubst literal-arg")
		subshell := shellshape.Normalize("envsubst $(dangerous-command)")
		if benign == subshell {
