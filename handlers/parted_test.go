...
func TestParted(t *testing.T) {
	...
	t.Run("different devices collide", func(t *testing.T) {
		a := shellshape.Normalize("parted /dev/sda print")
		b := shellshape.Normalize("parted /dev/nvme0n1 print")
		if a != b {
	...
	t.Run("different label types collide", func(t *testing.T) {
		a := shellshape.Normalize("parted /dev/sda mklabel gpt")
		b := shellshape.Normalize("parted /dev/sda mklabel msdos")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("parted /dev/sda print")
		subshell := shellshape.Normalize("parted $(dangerous-command) print")
		if benign == subshell {
