...
func TestHTTPie(t *testing.T) {
	...
	t.Run("different URLs collide", func(t *testing.T) {
		a := shellshape.Normalize("http GET https://api.example.com/users")
		b := shellshape.Normalize("http GET https://api.other.com/items")
		if a != b {
	...
	t.Run("different request items collide", func(t *testing.T) {
		a := shellshape.Normalize("http POST https://example.com name=Alice age:=30")
		b := shellshape.Normalize("http POST https://example.com name=Bob age:=25")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different auth collide", func(t *testing.T) {
		a := shellshape.Normalize("http -a alice:pass1 https://example.com")
		b := shellshape.Normalize("http -a bob:pass2 https://example.com")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("http https://example.com")
		subshell := shellshape.Normalize("http $(dangerous-command)")
		if benign == subshell {
