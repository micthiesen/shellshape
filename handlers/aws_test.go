...
func TestAws(t *testing.T) {
	...
	t.Run("different function names collide", func(t *testing.T) {
		a := shellshape.Normalize("aws lambda invoke --function-name myFunc out.json")
		b := shellshape.Normalize("aws lambda invoke --function-name otherFunc out.json")
		if a != b {
	...
	t.Run("different queries collide", func(t *testing.T) {
		a := shellshape.Normalize("aws ec2 describe-instances --query Reservations[].Instances[].InstanceId")
		b := shellshape.Normalize("aws ec2 describe-instances --query SecurityGroups[].GroupId")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different s3 paths collide", func(t *testing.T) {
		a := shellshape.Normalize("aws s3 cp data.csv s3://bucket-a/uploads/data.csv")
		b := shellshape.Normalize("aws s3 cp report.pdf s3://bucket-b/reports/report.pdf")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("aws lambda invoke --function-name myFunc out.json")
		subshell := shellshape.Normalize("aws lambda invoke --function-name $(get-func-name) out.json")
		if benign == subshell {
