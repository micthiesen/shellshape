...
func TestChmod(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := shellshape.Normalize("chmod 755 /home/alice/script.sh")
		b := shellshape.Normalize("chmod 755 /opt/deploy/run.sh")
		if a != b {
	...
	t.Run("different modes distinct", func(t *testing.T) {
		a := shellshape.Normalize("chmod 755 file.sh")
		b := shellshape.Normalize("chmod 644 file.sh")
		if a == b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("chmod 755 script.sh")
		subshell := shellshape.Normalize("chmod $(dangerous-command) script.sh")
		if benign == subshell {
