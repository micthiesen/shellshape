...
func init() {
	shellshape.Register("dotnet", handleDotnet, shellshape.HandlerOptions{HasSubcommands: true})
}
...
var dotnetCategories = []shellshape.FlagCategory{
	{Flags: dotnetPatternFlags, Placeholder: "<pattern>"},
	{Flags: dotnetPathFlags, Placeholder: "<path>"},
	{Flags: dotnetValFlags, Placeholder: "<val>"},
}
...
func handleDotnet(subcommand string, tokens []string) []string {
	...
	for i < len(args) {
		...
		if strings.Contains(tok, "=") && shellshape.IsFlagToken(tok) {
			if norm, ok := shellshape.ConsumeFusedFlag(tok, dotnetCategories); ok {
		...
		if placeholder, ok := shellshape.MatchFlagCategory(tok, dotnetCategories); ok {
