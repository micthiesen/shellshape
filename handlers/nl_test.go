...
func TestNl(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("nl -ba /var/log/syslog")
		b := shellshape.Normalize("nl -ba /tmp/output.txt")
		if a != b {
	...
	t.Run("different patterns collide", func(t *testing.T) {
		a := shellshape.Normalize("nl -b p'error' file.txt")
		b := shellshape.Normalize("nl -b p'warning' file.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different numbers collide", func(t *testing.T) {
		a := shellshape.Normalize("nl -v 1 -i 2 file.txt")
		b := shellshape.Normalize("nl -v 100 -i 10 file.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("nl literal-arg")
		subshell := shellshape.Normalize("nl $(dangerous-command)")
		if benign == subshell {
