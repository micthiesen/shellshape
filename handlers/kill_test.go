...
func TestKill(t *testing.T) {
	...
	t.Run("different pids collide", func(t *testing.T) {
		a := shellshape.Normalize("kill -9 1234")
		b := shellshape.Normalize("kill -9 5678")
		if a != b {
	...
	t.Run("different pid counts collide", func(t *testing.T) {
		a := shellshape.Normalize("kill 1234 5678")
		b := shellshape.Normalize("kill 111 222 333")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("kill 1234")
		subshell := shellshape.Normalize("kill $(dangerous-command)")
		if benign == subshell {
