...
func TestTsc(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("tsc src/foo.ts")
		b := shellshape.Normalize("tsc src/bar.ts")
		if a != b {
	...
	t.Run("different target values collide", func(t *testing.T) {
		a := shellshape.Normalize("tsc --target ES2020 src/index.ts")
		b := shellshape.Normalize("tsc --target ES2022 src/main.ts")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("tsc src/index.ts")
		subshell := shellshape.Normalize("tsc $(dangerous-command)")
		if benign == subshell {
