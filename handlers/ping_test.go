...
func TestPing(t *testing.T) {
	...
	t.Run("different hosts collide", func(t *testing.T) {
		a := shellshape.Normalize("ping -c 5 google.com")
		b := shellshape.Normalize("ping -c 5 yahoo.com")
		if a != b {
	...
	t.Run("different counts collide", func(t *testing.T) {
		a := shellshape.Normalize("ping -c 5 example.com")
		b := shellshape.Normalize("ping -c 100 example.com")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("ping google.com")
		subshell := shellshape.Normalize("ping $(dangerous-command)")
		if benign == subshell {
