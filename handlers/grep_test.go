...
func TestGrep(t *testing.T) {
	...
	t.Run("different patterns collide", func(t *testing.T) {
		a := shellshape.Normalize("grep -E '(foo|bar)' file")
		b := shellshape.Normalize("grep -E '(baz|qux)' file")
		if a != b {
	...
	t.Run("subshell not collapsed to pattern", func(t *testing.T) {
		benign := shellshape.Normalize("grep foo file.txt")
		subshell := shellshape.Normalize("grep $(cat pattern.txt) file.txt")
		if benign == subshell {
...
func TestGrepContext(t *testing.T) {
	...
	t.Run("different numbers collide", func(t *testing.T) {
		a := shellshape.Normalize("grep -A 5 foo file.txt")
		b := shellshape.Normalize("grep -A 20 bar file.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
