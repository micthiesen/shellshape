...
func TestIptables(t *testing.T) {
	...
	t.Run("different IPs collide", func(t *testing.T) {
		a := shellshape.Normalize("iptables -A INPUT -s 10.0.0.1 -j DROP")
		b := shellshape.Normalize("iptables -A INPUT -s 172.16.5.99 -j DROP")
		if a != b {
	...
	t.Run("different ports collide", func(t *testing.T) {
		a := shellshape.Normalize("iptables -A INPUT -p tcp --dport 80 -j ACCEPT")
		b := shellshape.Normalize("iptables -A INPUT -p tcp --dport 443 -j ACCEPT")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("iptables -A INPUT -s 10.0.0.1 -j DROP")
		subshell := shellshape.Normalize("iptables -A INPUT -s $(get-ip) -j DROP")
		if benign == subshell {
