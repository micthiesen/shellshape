...
func TestWatch(t *testing.T) {
	...
	t.Run("different intervals collide", func(t *testing.T) {
		a := shellshape.Normalize("watch -n 1 ls")
		b := shellshape.Normalize("watch -n 60 ls")
		if a != b {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := shellshape.Normalize("watch -n 5 cat /etc/hosts")
		b := shellshape.Normalize("watch -n 5 cat /var/log/syslog")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("watch ls")
		subshell := shellshape.Normalize("watch $(dangerous-command)")
		if benign == subshell {
