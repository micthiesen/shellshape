...
func TestIconv(t *testing.T) {
	...
	t.Run("different encodings collide", func(t *testing.T) {
		a := shellshape.Normalize("iconv -f ISO-8859-1 -t UTF-8 input.txt")
		b := shellshape.Normalize("iconv -f SHIFT_JIS -t ASCII input.txt")
		if a != b {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("iconv -f UTF-8 -t ASCII data.csv")
		b := shellshape.Normalize("iconv -f UTF-8 -t ASCII report.json")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("iconv -f UTF-8 literal.txt")
		subshell := shellshape.Normalize("iconv -f UTF-8 $(dangerous-command)")
		if benign == subshell {
