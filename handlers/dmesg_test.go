...
func TestDmesg(t *testing.T) {
	...
	t.Run("different levels collide", func(t *testing.T) {
		a := shellshape.Normalize("dmesg --level err")
		b := shellshape.Normalize("dmesg --level warn")
		if a != b {
	...
	t.Run("different facilities collide", func(t *testing.T) {
		a := shellshape.Normalize("dmesg -f kern")
		b := shellshape.Normalize("dmesg -f daemon,user")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different buffer sizes collide", func(t *testing.T) {
		a := shellshape.Normalize("dmesg -s 8192")
		b := shellshape.Normalize("dmesg -s 65536")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("dmesg -F /var/log/kern.log")
		subshell := shellshape.Normalize("dmesg -F $(dangerous-command)")
		if benign == subshell {
