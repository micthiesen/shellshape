...
func TestTcpdump(t *testing.T) {
	...
	t.Run("different hosts collide", func(t *testing.T) {
		a := shellshape.Normalize("tcpdump -i eth0 host 192.168.1.1")
		b := shellshape.Normalize("tcpdump -i eth0 host 10.0.0.1")
		if a != b {
	...
	t.Run("different ports collide", func(t *testing.T) {
		a := shellshape.Normalize("tcpdump port 80")
		b := shellshape.Normalize("tcpdump port 443")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different pcap files collide", func(t *testing.T) {
		a := shellshape.Normalize("tcpdump -w /tmp/a.pcap")
		b := shellshape.Normalize("tcpdump -w /tmp/b.pcap")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("tcpdump -i eth0")
		subshell := shellshape.Normalize("tcpdump -i $(get-interface)")
		if benign == subshell {
