...
func TestPasswd(t *testing.T) {
	...
	t.Run("different users collide", func(t *testing.T) {
		a := shellshape.Normalize("passwd -l alice")
		b := shellshape.Normalize("passwd -l bob")
		if a != b {
	...
	t.Run("different days collide", func(t *testing.T) {
		a := shellshape.Normalize("passwd -x 90 alice")
		b := shellshape.Normalize("passwd -x 365 bob")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("passwd alice")
		subshell := shellshape.Normalize("passwd $(dangerous-command)")
		if benign == subshell {
