...
func TestCrontab(t *testing.T) {
	...
	t.Run("different users collide", func(t *testing.T) {
		a := shellshape.Normalize("crontab -u alice -l")
		b := shellshape.Normalize("crontab -u bob -l")
		if a != b {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("crontab /etc/cron.d/job1")
		b := shellshape.Normalize("crontab /tmp/mycron.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("crontab mycron.txt")
		subshell := shellshape.Normalize("crontab $(dangerous-command)")
		if benign == subshell {
