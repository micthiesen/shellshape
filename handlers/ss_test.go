...
func TestSs(t *testing.T) {
	...
	t.Run("different addresses collide", func(t *testing.T) {
		a := shellshape.Normalize("ss -t dst 192.168.1.1:80")
		b := shellshape.Normalize("ss -t dst 10.0.0.1:443")
		if a != b {
	...
	t.Run("different states collide", func(t *testing.T) {
		a := shellshape.Normalize("ss state LISTEN")
		b := shellshape.Normalize("ss state ESTABLISHED")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different filters collide", func(t *testing.T) {
		a := shellshape.Normalize("ss 'sport = :80'")
		b := shellshape.Normalize("ss 'dport = :443'")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("ss -t dst 192.168.1.1:80")
		subshell := shellshape.Normalize("ss -t dst $(echo evil)")
		if benign == subshell {
