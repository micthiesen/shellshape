...
func TestDiff(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := shellshape.Normalize("diff -u config.yaml settings.yaml")
		b := shellshape.Normalize("diff -u main.go util.go")
		if a != b {
	...
	t.Run("different context numbers collide", func(t *testing.T) {
		a := shellshape.Normalize("diff -C 3 file1.txt file2.txt")
		b := shellshape.Normalize("diff -C 10 old.txt new.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := shellshape.Normalize("diff file1.txt file2.txt")
		subshell := shellshape.Normalize("diff $(dangerous-command) file2.txt")
		if benign == subshell {
