...
func TestDu(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := Normalize("du -sh /home/alice")
		b := Normalize("du -sh /home/bob")
		if a != b {
	...
	t.Run("different depths collide", func(t *testing.T) {
		a := Normalize("du -d 1 /var")
		b := Normalize("du -d 3 /tmp")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("du -sh /home/user")
		subshell := Normalize("du -sh $(pwd)")
		if benign == subshell {
