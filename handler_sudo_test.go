...
func TestSudo(t *testing.T) {
	...
	t.Run("different users collide", func(t *testing.T) {
		a := Normalize("sudo -u alice ls /home")
		b := Normalize("sudo -u bob ls /home")
		if a != b {
	...
	t.Run("different inner paths collide", func(t *testing.T) {
		a := Normalize("sudo cat /etc/shadow")
		b := Normalize("sudo cat /etc/passwd")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("sudo literal-arg")
		subshell := Normalize("sudo $(dangerous-command)")
		if benign == subshell {
