...
func TestMount(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := Normalize("mount /dev/sda1 /mnt/disk1")
		b := Normalize("mount /dev/sdb2 /mnt/disk2")
		if a != b {
	...
	t.Run("different options collide", func(t *testing.T) {
		a := Normalize("mount -o rw,noexec /dev/sda1 /data")
		b := Normalize("mount -o ro,nosuid /dev/sdb1 /srv")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("mount /dev/sda1 /mnt")
		subshell := Normalize("mount $(dangerous-command) /mnt")
		if benign == subshell {
...
func TestUmount(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := Normalize("umount /mnt/disk1")
		b := Normalize("umount /mnt/disk2")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	// SAFETY TEST: subshell must not collapse
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("umount /mnt")
		subshell := Normalize("umount $(dangerous-command)")
		if benign == subshell {
			t.Error("subshell must produce different shape than literal")
		}
	})
