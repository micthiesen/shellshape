...
func TestUniq(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("uniq -c /var/log/syslog")
		b := Normalize("uniq -c /tmp/data.txt")
		if a != b {
	...
	t.Run("different skip numbers collide", func(t *testing.T) {
		a := Normalize("uniq -f 3 file.txt")
		b := Normalize("uniq -f 10 other.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("uniq literal-arg")
		subshell := Normalize("uniq $(dangerous-command)")
		if benign == subshell {
