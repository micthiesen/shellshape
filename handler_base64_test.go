...
func TestBase64(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("base64 /tmp/secret.bin")
		b := Normalize("base64 /home/user/photo.jpg")
		if a != b {
	...
	t.Run("different output paths collide", func(t *testing.T) {
		a := Normalize("base64 -o /tmp/a.txt /tmp/input1.txt")
		b := Normalize("base64 -o /home/user/b.txt /var/data.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("base64 literal-arg")
		subshell := Normalize("base64 $(dangerous-command)")
		if benign == subshell {
