...
func TestDeno(t *testing.T) {
	...
	t.Run("different scripts collide", func(t *testing.T) {
		a := Normalize("deno run server.ts")
		b := Normalize("deno run client.ts")
		if a != b {
	...
	t.Run("different eval code collides", func(t *testing.T) {
		a := Normalize("deno eval 'console.log(1)'")
		b := Normalize("deno eval 'Deno.exit(0)'")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different config paths collide", func(t *testing.T) {
		a := Normalize("deno run --config deno.json app.ts")
		b := Normalize("deno run --config tsconfig.json app.ts")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("deno run app.ts")
		subshell := Normalize("deno run $(dangerous-command)")
		if benign == subshell {
