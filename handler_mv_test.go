...
func TestMv(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("mv /tmp/a.txt /tmp/b.txt")
		b := Normalize("mv config.yaml backup.yaml")
		c := Normalize("mv ~/doc.pdf ~/archive/doc.pdf")
		if a != b || b != c {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("mv literal-arg dest/")
		subshell := Normalize("mv $(dangerous-command) dest/")
		if benign == subshell {
