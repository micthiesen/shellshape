...
func TestColumn(t *testing.T) {
	...
	t.Run("different delimiters collide", func(t *testing.T) {
		a := Normalize("column -t -s ',' file.csv")
		b := Normalize("column -t -s ':' file.csv")
		if a != b {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("column -t data.csv")
		b := Normalize("column -t report.tsv")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("column literal-arg")
		subshell := Normalize("column $(dangerous-command)")
		if benign == subshell {
