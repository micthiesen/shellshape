...
func TestMongo(t *testing.T) {
	...
	t.Run("different databases collide", func(t *testing.T) {
		a := Normalize("mongosh production")
		b := Normalize("mongosh staging")
		if a != b {
	...
	t.Run("different hosts collide", func(t *testing.T) {
		a := Normalize("mongo --host 10.0.0.1 --port 27017 mydb")
		b := Normalize("mongo --host 192.168.1.5 --port 5555 otherdb")
		if a != b {
			t.Errorf("expected same shape, got %q vs %q", a, b)
		}
	})
	...
	t.Run("different eval expressions collide", func(t *testing.T) {
		a := Normalize("mongosh --eval 'db.users.find()'")
		b := Normalize("mongosh --eval 'db.orders.count()'")
		if a != b {
			t.Errorf("expected same shape, got %q vs %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("mongosh literal-arg")
		subshell := Normalize("mongosh $(dangerous-command)")
		if benign == subshell {
