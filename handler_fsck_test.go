...
func TestFsck(t *testing.T) {
	...
	t.Run("different devices collide", func(t *testing.T) {
		a := Normalize("fsck /dev/sda1")
		b := Normalize("fsck /dev/nvme0n1p3")
		if a != b {
	...
	t.Run("different types collide", func(t *testing.T) {
		a := Normalize("fsck -t ext4 /dev/sda1")
		b := Normalize("fsck -t xfs /dev/vda1")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("fsck /dev/sda1")
		subshell := Normalize("fsck $(dangerous-command)")
		if benign == subshell {
