...
func TestPacman(t *testing.T) {
	...
	t.Run("different search terms collide", func(t *testing.T) {
		a := Normalize("pacman -Ss docker")
		b := Normalize("pacman -Ss kubernetes")
		if a != b {
	...
	t.Run("different local packages collide", func(t *testing.T) {
		a := Normalize("pacman -U /tmp/foo-1.0.pkg.tar.zst")
		b := Normalize("pacman -U /tmp/bar-2.0.pkg.tar.zst")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("pacman -S literal-arg")
		subshell := Normalize("pacman -S $(dangerous-command)")
		if benign == subshell {
