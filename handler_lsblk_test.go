...
func TestLsblk(t *testing.T) {
	...
	t.Run("different devices collide", func(t *testing.T) {
		a := Normalize("lsblk /dev/sda")
		b := Normalize("lsblk /dev/nvme0n1")
		if a != b {
	...
	t.Run("different columns collide", func(t *testing.T) {
		a := Normalize("lsblk -o NAME,SIZE")
		b := Normalize("lsblk -o UUID,LABEL,MOUNTPOINT")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("lsblk /dev/sda")
		subshell := Normalize("lsblk $(dangerous-command)")
		if benign == subshell {
