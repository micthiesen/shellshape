...
func TestNpx(t *testing.T) {
	...
	t.Run("different args collide", func(t *testing.T) {
		a := Normalize("npx prettier --write src/app.ts")
		b := Normalize("npx prettier --write lib/index.js")
		if a != b {
	...
	t.Run("different packages collide", func(t *testing.T) {
		a := Normalize("npx -p typescript tsc")
		b := Normalize("npx -p @types/node tsc")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("npx cowsay hello")
		subshell := Normalize("npx cowsay $(dangerous-command)")
		if benign == subshell {
