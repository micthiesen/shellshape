...
func TestDotenvx(t *testing.T) {
	...
	t.Run("different env files collide", func(t *testing.T) {
		a := Normalize("dotenvx run -f .env.production -- node index.js")
		b := Normalize("dotenvx run -f .env.staging -- node index.js")
		if a != b {
	...
	t.Run("different env values collide", func(t *testing.T) {
		a := Normalize("dotenvx run -e HELLO=World -- node index.js")
		b := Normalize("dotenvx run -e DB_HOST=localhost -- node index.js")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different commands after -- collide on paths", func(t *testing.T) {
		a := Normalize("dotenvx run -- node /app/server.js")
		b := Normalize("dotenvx run -- node /app/client.js")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("dotenvx run -f .env -- node index.js")
		subshell := Normalize("dotenvx run -f $(dangerous-command) -- node index.js")
		if benign == subshell {
