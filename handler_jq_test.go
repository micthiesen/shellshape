...
func TestJq(t *testing.T) {
	...
	t.Run("different filters collide", func(t *testing.T) {
		a := Normalize("jq '.name' data.json")
		b := Normalize("jq '.age' data.json")
		if a != b {
	...
	t.Run("different arg values collide", func(t *testing.T) {
		a := Normalize("jq --arg key hello '.' data.json")
		b := Normalize("jq --arg user world '.' data.json")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("jq '.name' file.json")
		subshell := Normalize("jq $(dangerous-command) file.json")
		if benign == subshell {
