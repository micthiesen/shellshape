...
func TestSwift(t *testing.T) {
	...
	t.Run("different configurations collide", func(t *testing.T) {
		a := Normalize("swift build -c release")
		b := Normalize("swift build -c debug")
		if a != b {
	...
	t.Run("different test filters collide", func(t *testing.T) {
		a := Normalize("swift test --filter UnitTests")
		b := Normalize("swift test --filter IntegrationTests")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different package paths collide", func(t *testing.T) {
		a := Normalize("swift build --package-path /Users/alice/proj")
		b := Normalize("swift build --package-path /Users/bob/proj")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("swift build --product MyApp")
		subshell := Normalize("swift build --product $(dangerous-command)")
		if benign == subshell {
...
func TestSwiftc(t *testing.T) {
	...
	t.Run("different source files collide", func(t *testing.T) {
		a := Normalize("swiftc foo.swift")
		b := Normalize("swiftc bar.swift")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different output paths collide", func(t *testing.T) {
		a := Normalize("swiftc -o /tmp/app1 main.swift")
		b := Normalize("swiftc -o /tmp/app2 main.swift")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	// SAFETY TEST
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("swiftc main.swift")
		subshell := Normalize("swiftc $(dangerous-command)")
		if benign == subshell {
			t.Error("subshell must produce different shape than literal")
		}
	})
