...
func TestStat(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("stat /etc/passwd")
		b := Normalize("stat /var/log/syslog")
		if a != b {
	...
	t.Run("different format strings collide", func(t *testing.T) {
		a := Normalize("stat -f '%z' file1.txt")
		b := Normalize("stat -f '%Sm' file2.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("stat literal-arg")
		subshell := Normalize("stat $(dangerous-command)")
		if benign == subshell {
