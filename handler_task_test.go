...
func TestTask(t *testing.T) {
	...
	t.Run("different dirs collide", func(t *testing.T) {
		a := Normalize("task -d /home/alice/project build")
		b := Normalize("task -d /home/bob/project build")
		if a != b {
	...
	t.Run("different variables collide", func(t *testing.T) {
		a := Normalize("task deploy ENV=staging VERSION=1.0")
		b := Normalize("task deploy ENV=prod VERSION=2.5")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("task build")
		subshell := Normalize("task $(dangerous-command)")
		if benign == subshell {
