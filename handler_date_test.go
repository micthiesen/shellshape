...
func TestDate(t *testing.T) {
	...
	t.Run("different formats collide", func(t *testing.T) {
		a := Normalize("date +%Y-%m-%d")
		b := Normalize("date +%H:%M:%S")
		if a != b {
	...
	t.Run("different date strings collide", func(t *testing.T) {
		a := Normalize(`date -d "2024-01-01"`)
		b := Normalize(`date -d "last friday"`)
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("date +%Y-%m-%d")
		subshell := Normalize("date $(dangerous-command)")
		if benign == subshell {
