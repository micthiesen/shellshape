...
func TestUnzip(t *testing.T) {
	...
	t.Run("different archives collide", func(t *testing.T) {
		a := Normalize("unzip project-v1.zip")
		b := Normalize("unzip backup-2024.zip")
		if a != b {
	...
	t.Run("different destinations collide", func(t *testing.T) {
		a := Normalize("unzip archive.zip -d /home/alice/docs")
		b := Normalize("unzip archive.zip -d /var/data/output")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("unzip archive.zip")
		subshell := Normalize("unzip $(dangerous-command)")
		if benign == subshell {
