...
func TestWhois(t *testing.T) {
	...
	t.Run("different domains collide", func(t *testing.T) {
		a := Normalize("whois google.com")
		b := Normalize("whois example.org")
		if a != b {
	...
	t.Run("different hosts collide", func(t *testing.T) {
		a := Normalize("whois -h whois.arin.net example.com")
		b := Normalize("whois -h whois.ripe.net example.org")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("whois example.com")
		subshell := Normalize("whois $(dangerous-command)")
		if benign == subshell {
