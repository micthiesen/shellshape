...
func TestGroupadd(t *testing.T) {
	...
	t.Run("different groups collide", func(t *testing.T) {
		a := Normalize("groupadd developers")
		b := Normalize("groupadd marketing")
		if a != b {
	...
	t.Run("different gids collide", func(t *testing.T) {
		a := Normalize("groupadd -g 1001 devs")
		b := Normalize("groupadd -g 9999 ops")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("groupadd mygroup")
		subshell := Normalize("groupadd $(dangerous-command)")
		if benign == subshell {
...
func TestGroupmod(t *testing.T) {
	...
	// SAFETY TEST: subshell must not collapse
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("groupmod -n newname oldgroup")
		subshell := Normalize("groupmod -n $(dangerous-command) oldgroup")
		if benign == subshell {
			t.Error("subshell must produce different shape than literal")
		}
	})
...
func TestGroupdel(t *testing.T) {
	...
	t.Run("different groups collide", func(t *testing.T) {
		a := Normalize("groupdel marketing")
		b := Normalize("groupdel engineering")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	// SAFETY TEST: subshell must not collapse
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("groupdel mygroup")
		subshell := Normalize("groupdel $(dangerous-command)")
		if benign == subshell {
			t.Error("subshell must produce different shape than literal")
		}
	})
