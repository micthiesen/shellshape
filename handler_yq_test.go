...
func TestYq(t *testing.T) {
	...
	t.Run("different filters collide", func(t *testing.T) {
		a := Normalize("yq '.name' data.yaml")
		b := Normalize("yq '.age' data.yaml")
		if a != b {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("yq '.' config.yaml")
		b := Normalize("yq '.' settings.yml")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("yq '.name' file.yaml")
		subshell := Normalize("yq $(dangerous-command) file.yaml")
		if benign == subshell {
