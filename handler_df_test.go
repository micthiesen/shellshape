...
func TestDf(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := Normalize("df -h /home/alice")
		b := Normalize("df -h /home/bob")
		if a != b {
	...
	t.Run("different types collide", func(t *testing.T) {
		a := Normalize("df -T ext4")
		b := Normalize("df -T nfs")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("df /home/user")
		subshell := Normalize("df $(dangerous-command)")
		if benign == subshell {
