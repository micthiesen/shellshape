...
func TestOpenssl(t *testing.T) {
	...
	t.Run("different paths collide", func(t *testing.T) {
		a := Normalize("openssl x509 -in /tmp/cert1.pem -text -noout")
		b := Normalize("openssl x509 -in /etc/ssl/cert2.pem -text -noout")
		if a != b {
	...
	t.Run("different keys collide", func(t *testing.T) {
		a := Normalize("openssl genrsa -out mykey.key 4096")
		b := Normalize("openssl genrsa -out otherkey.key 2048")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("openssl dgst -sha256 file.txt")
		subshell := Normalize("openssl dgst -sha256 $(find . -name cert.pem)")
		if benign == subshell {
