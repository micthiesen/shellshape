...
func TestTraceroute(t *testing.T) {
	...
	t.Run("different hosts collide", func(t *testing.T) {
		a := Normalize("traceroute -m 30 google.com")
		b := Normalize("traceroute -m 30 yahoo.com")
		if a != b {
	...
	t.Run("different packet sizes collide", func(t *testing.T) {
		a := Normalize("traceroute example.com 60")
		b := Normalize("traceroute example.com 1500")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("traceroute example.com")
		subshell := Normalize("traceroute $(dangerous-command)")
		if benign == subshell {
