...
func TestXxd(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("xxd -l 120 secret.bin")
		b := Normalize("xxd -l 256 firmware.dat")
		if a != b {
	...
	t.Run("different numeric values collide", func(t *testing.T) {
		a := Normalize("xxd -s 0x30 -c 16 file.bin")
		b := Normalize("xxd -s 0xFF -c 32 data.bin")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("xxd literal-arg")
		subshell := Normalize("xxd $(dangerous-command)")
		if benign == subshell {
