...
func TestSort(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("sort -k 2,3 data.csv")
		b := Normalize("sort -k 2,3 results.tsv")
		if a != b {
	...
	t.Run("different keys collide", func(t *testing.T) {
		a := Normalize("sort -k 1,1 file.txt")
		b := Normalize("sort -k 3,5nr file.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("sort literal-arg")
		subshell := Normalize("sort $(dangerous-command)")
		if benign == subshell {
