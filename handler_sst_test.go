...
func TestSst(t *testing.T) {
	...
	t.Run("different stages do not collide", func(t *testing.T) {
		a := Normalize("sst deploy --stage prod")
		b := Normalize("sst deploy --stage dev")
		if a == b {
	...
	t.Run("different profiles collide", func(t *testing.T) {
		a := Normalize("sst deploy --stage prod --profile default")
		b := Normalize("sst deploy --stage prod --profile company-sso")
		if a != b {
	...
	t.Run("different secret values collide", func(t *testing.T) {
		a := Normalize("sst secret set MyKey value-one")
		b := Normalize("sst secret set MyKey value-two")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("sst deploy --stage prod")
		subshell := Normalize("sst deploy $(dangerous-command)")
		if benign == subshell {
