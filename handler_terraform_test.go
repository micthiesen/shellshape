...
func TestTerraform(t *testing.T) {
	...
	t.Run("different var values collide", func(t *testing.T) {
		a := Normalize("terraform plan -var 'region=us-east-1'")
		b := Normalize("terraform plan -var 'region=eu-west-1'")
		if a != b {
	...
	t.Run("different var-file paths collide", func(t *testing.T) {
		a := Normalize("terraform plan -var-file prod.tfvars")
		b := Normalize("terraform plan -var-file staging.tfvars")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different target resources collide", func(t *testing.T) {
		a := Normalize("terraform destroy -target aws_instance.web")
		b := Normalize("terraform destroy -target aws_s3_bucket.data")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different import args collide", func(t *testing.T) {
		a := Normalize("terraform import aws_instance.web i-abc123")
		b := Normalize("terraform import aws_s3_bucket.data my-bucket")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("terraform apply ./plan.out")
		subshell := Normalize("terraform apply $(dangerous-command)")
		if benign == subshell {
