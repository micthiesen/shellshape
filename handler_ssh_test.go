...
func TestSsh(t *testing.T) {
	...
	t.Run("different hosts collide", func(t *testing.T) {
		a := Normalize("ssh production-server")
		b := Normalize("ssh staging-server")
		if a != b {
	...
	t.Run("different remote commands collide", func(t *testing.T) {
		a := Normalize("ssh myserver cat /etc/hosts")
		b := Normalize("ssh otherserver tail -f /var/log/syslog")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different ports collide", func(t *testing.T) {
		a := Normalize("ssh -p 22 myserver")
		b := Normalize("ssh -p 2222 otherserver")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("ssh literal-host")
		subshell := Normalize("ssh $(dangerous-command)")
		if benign == subshell {
