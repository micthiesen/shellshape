...
func TestLn(t *testing.T) {
	...
	t.Run("different targets collide", func(t *testing.T) {
		a := Normalize("ln -s /usr/src /home/src")
		b := Normalize("ln -s /opt/app /var/link")
		c := Normalize("ln -s ~/dotfiles/.bashrc ~/.bashrc")
		if a != b || b != c {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("ln literal-arg")
		subshell := Normalize("ln $(dangerous-command)")
		if benign == subshell {
