...
func TestMtr(t *testing.T) {
	...
	t.Run("different hosts collide", func(t *testing.T) {
		a := Normalize("mtr -c 10 google.com")
		b := Normalize("mtr -c 10 yahoo.com")
		if a != b {
	...
	t.Run("different cycle counts collide", func(t *testing.T) {
		a := Normalize("mtr -c 5 example.com")
		b := Normalize("mtr -c 100 example.com")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("mtr example.com")
		subshell := Normalize("mtr $(dangerous-command)")
		if benign == subshell {
