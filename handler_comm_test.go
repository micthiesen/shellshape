...
func TestComm(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("comm -12 names.txt ids.txt")
		b := Normalize("comm -12 config.yaml settings.yaml")
		if a != b {
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("comm file1.txt file2.txt")
		subshell := Normalize("comm $(dangerous-command) file2.txt")
		if benign == subshell {
