...
func TestScreen(t *testing.T) {
	...
	t.Run("different session names collide", func(t *testing.T) {
		a := Normalize("screen -S project-alpha")
		b := Normalize("screen -S project-beta")
		if a != b {
	...
	t.Run("different reattach targets collide", func(t *testing.T) {
		a := Normalize("screen -r session1")
		b := Normalize("screen -r session2")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different scrollback sizes collide", func(t *testing.T) {
		a := Normalize("screen -h 1000")
		b := Normalize("screen -h 9999")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("screen literal-arg")
		subshell := Normalize("screen $(dangerous-command)")
		if benign == subshell {
