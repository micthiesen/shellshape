...
func TestEcho(t *testing.T) {
	...
	t.Run("different contents collide", func(t *testing.T) {
		a := Normalize("echo backup created")
		b := Normalize("echo rerere enabled")
		c := Normalize("echo --- main changes ---")
		if a != b || b != c {
	...
	t.Run("subshell not collapsed to str", func(t *testing.T) {
		benign := Normalize("echo hello")
		subshell := Normalize("echo $(rm -rf /tmp/foo)")
		if benign == subshell {
