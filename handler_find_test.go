...
func TestFind(t *testing.T) {
	...
	t.Run("different globs collide", func(t *testing.T) {
		a := Normalize("find /src -name '*buyside*' -o -name '*Buyside*' 2>/dev/null")
		b := Normalize("find /src -name '*notifications*' -o -name '*modelReady*' 2>/dev/null")
		if a != b {
	...
	t.Run("complex with or", func(t *testing.T) {
		got := Normalize("find /src -path '*notifications*' -name '*modelReady*' -o -path '*notifications*' -name '*modelNotSupported*' | head -10")
