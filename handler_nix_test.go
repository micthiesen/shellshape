...
func TestNix(t *testing.T) {
	...
	t.Run("different flake refs collide", func(t *testing.T) {
		a := Normalize("nix build nixpkgs#hello")
		b := Normalize("nix build nixpkgs#cowsay")
		if a != b {
	...
	t.Run("different local flake refs collide", func(t *testing.T) {
		a := Normalize("nix build .#myapp")
		b := Normalize("nix build .#otherapp")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different search terms collide", func(t *testing.T) {
		a := Normalize("nix search nixpkgs python")
		b := Normalize("nix search nixpkgs nodejs")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("nix build literal-arg")
		subshell := Normalize("nix build $(dangerous-command)")
		if benign == subshell {
