...
func TestTmux(t *testing.T) {
	...
	t.Run("different session names collide", func(t *testing.T) {
		a := Normalize("tmux new-session -s project-alpha")
		b := Normalize("tmux new-session -s project-beta")
		if a != b {
	...
	t.Run("different targets collide", func(t *testing.T) {
		a := Normalize("tmux attach -t session1")
		b := Normalize("tmux attach -t session2")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different send-keys values collide", func(t *testing.T) {
		a := Normalize("tmux send-keys -t dev 'make build' Enter")
		b := Normalize("tmux send-keys -t dev 'npm test' C-m")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("tmux send-keys -t dev literal-arg")
		subshell := Normalize("tmux send-keys -t dev $(dangerous-command)")
		if benign == subshell {
