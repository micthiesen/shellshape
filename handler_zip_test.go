...
func TestZip(t *testing.T) {
	...
	t.Run("different archives collide", func(t *testing.T) {
		a := Normalize("zip -r project-v1.zip src/")
		b := Normalize("zip -r backup-2024.zip src/")
		if a != b {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("zip archive.zip README.md config.yaml")
		b := Normalize("zip archive.zip main.go utils.py")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("zip archive.zip file.txt")
		subshell := Normalize("zip $(dangerous-command) file.txt")
		if benign == subshell {
