...
func TestCmake(t *testing.T) {
	...
	t.Run("different -D values collide", func(t *testing.T) {
		a := Normalize("cmake -DCMAKE_BUILD_TYPE=Release .")
		b := Normalize("cmake -DCMAKE_BUILD_TYPE=Debug .")
		if a != b {
	...
	t.Run("different generators collide", func(t *testing.T) {
		a := Normalize(`cmake -G "Unix Makefiles" .`)
		b := Normalize(`cmake -G Ninja .`)
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("different build directories collide", func(t *testing.T) {
		a := Normalize("cmake --build /home/user/project/build")
		b := Normalize("cmake --build /tmp/other-build")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("cmake literal-arg")
		subshell := Normalize("cmake $(dangerous-command)")
		if benign == subshell {
