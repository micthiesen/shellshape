...
func TestDpkg(t *testing.T) {
	...
	t.Run("different deb files collide", func(t *testing.T) {
		a := Normalize("dpkg -i ./foo_1.0_amd64.deb")
		b := Normalize("dpkg -i /tmp/bar_2.3_all.deb")
		if a != b {
	...
	t.Run("different search patterns collide", func(t *testing.T) {
		a := Normalize("dpkg -S /usr/bin/awk")
		b := Normalize("dpkg -S /usr/local/bin/python3")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("dpkg -i ./package.deb")
		subshell := Normalize("dpkg -i $(dangerous-command)")
		if benign == subshell {
