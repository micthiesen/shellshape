...
func TestHead(t *testing.T) {
	...
	t.Run("different files collide", func(t *testing.T) {
		a := Normalize("head -n 10 server.log")
		b := Normalize("head -n 10 access.log")
		if a != b {
	...
	t.Run("different counts collide", func(t *testing.T) {
		a := Normalize("head -n 5 file.txt")
		b := Normalize("head -n 100 file.txt")
		if a != b {
			t.Errorf("expected %q == %q", a, b)
		}
	})
	...
	t.Run("subshell not collapsed", func(t *testing.T) {
		benign := Normalize("head literal-arg")
		subshell := Normalize("head $(dangerous-command)")
		if benign == subshell {
